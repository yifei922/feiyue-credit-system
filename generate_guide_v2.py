...
def build_body(path, diags):
    ...
    story.append(Paragraph("• <b>支持任意格式</b>：图片（JPG/PNG/HEIC）、视频（MP4/MOV）、音频（MP3/M4A/WAV 等）、Word、Excel、PPT、PDF、压缩包等均可上传，单文件上限 <b>30MB</b>（系统会自动压缩以减小体积）。", S["body"]))
    ...
    faqs=[
          ...
          ("上传附件都支持哪些格式？","几乎支持任意格式：手机照片（JPG/PNG/HEIC）、视频（MP4/MOV）、音频（MP3/M4A/WAV/录音等）、Word、Excel、PPT、PDF、压缩包等都可以，单个文件最大 30MB。"),
          ("上传的音频（朗读/录音）会压缩吗？","会。音频（如 WAV/FLAC 等无损大文件）会自动转成 AAC 格式，体积可降 80% 左右，听感几乎无差别；已是 MP3 的也尽量在不损失听感的前提下压小，方便上传和下载。"),
          ("性能怎么样？20 个学生同时上传会卡吗？","不会卡。2026 暑期压测实测：20 个学生同时上传视频/音频/PDF/PPT/Word/图片混合附件，0 错误、全部成功。其中音频自动压缩 82%（258KB→48KB），PDF 自动压缩 80%（pdf-lib 对象流 + gzip），视频转码为网页友好的 H.264 格式。免费服务器只有 0.1 CPU，因此视频/音频等需要转码的文件会一个一个排队处理（4 秒视频约 2–7 秒），图片 / PDF / Word 等秒级完成。前端进度条实时显示「上传中 X%」与「处理中 XX%」两段进度，不会让人误以为卡住。"),
          ("用手机拍的作业照片怎么交？","在「提交作业」页直接点「选择文件」选中手机相册里的照片即可，系统会自动压缩体积（画质不变），不用自己先处理。"),
          ("上传后进度条一直在转 / 不动，正常吗？","正常。图片、PDF 压缩通常在瞬间完成，进度条一闪而过；只有「视频」「音频」转码较慢（免费版算力有限，较长视频可能需 1–3 分钟），此时进度条会显示真实百分比并平滑走到 100%，请耐心等待不要关闭页面。"),
          ("文件太大传不上去怎么办？","系统会自动压缩以减小体积：图片保留清晰度、视频做视觉无损瘦身。单文件上限 30 MB，若仍超限，请先在手机上对视频做简单剪辑/压缩（手机相册自带的「压缩视频」即可），或用电脑 WinRAR / macOS 归档工具压缩后再上传。"),
