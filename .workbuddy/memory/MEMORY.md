...
- ⚠️ **分页默认值 20 条是隐藏陷阱**（2026-09-07）：`server/src/util.js` 的 `paginate()` 默认 `pageSize=20`。所有 `listStudents()`/`listTasks()`/`listSubjects()` 调用**必须显式传 `{ pageSize: 200 }`**，否则永远只拉回第 1 页（20条），54 名学生只显示 20 人。已修复 6 个视图的调用，**新增同类 API 调用时也要注意**。
