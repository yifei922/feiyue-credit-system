...
router.get('/', async (req, res) => {
  ...
  const recs = [];
  const comps = await db.prepare("SELECT * FROM completion_record WHERE student_id=? AND status IN ('UNFINISHED','FAILED')").all(studentId);
  for (const c of comps) {
    const t = await db.prepare('SELECT * FROM task WHERE id=?').get(c.task_id);
    if (!t) continue;
    recs.push({
      taskId: t.id,
      title: t.title,
      subjectName: await db.prepare('SELECT name FROM subject WHERE id=?').get(t.subject_id)?.name || '',
      type: t.type,
