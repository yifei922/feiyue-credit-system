...
async function scopeTasks(user) {
  if (user.role === 'REP') {
    const ids = await getManagedSubjectIds(user);
    if (ids.length === 0) return [];
    return await db.prepare(`SELECT * FROM task WHERE subject_id IN (${ids.map(() => '?').join(',')})`).all(...ids);
  }
  return await db.prepare('SELECT * FROM task').all();
}
...
router.get('/overview', async (req, res) => {
  ...
  if (req.user.role === 'REP') {
    ...
    subjects = ids.length ? await db.prepare(`SELECT * FROM subject WHERE id IN (${ids.map(() => '?').join(',')})`).all(...ids) : [];
  } else {
    subjects = await db.prepare('SELECT * FROM subject').all();
  }
  const subjectCompletionRate = await Promise.all(subjects.map(async s => {
    const recs = await db.prepare(`SELECT cr.* FROM completion_record cr JOIN task t ON cr.task_id=t.id WHERE t.subject_id=?`).all(s.id);
  ...
  const allComp = req.user.role === 'REP'
    ? await db.prepare(distSql).all(...await getManagedSubjectIds(req.user))
    : await db.prepare(distSql).all();
  ...
  const studentCount = await (await db.prepare('SELECT COUNT(*) AS c FROM student').get()).c;
  const taskCount = tasks.length;
  const avgRow = await db.prepare('SELECT COALESCE(AVG(total_credits),0) AS a FROM student').get();
  const summary = {
    studentCount,
    taskCount,
...
router.get('/completion-summary', async (req, res) => {
  ...
  const tasks = await db.prepare(taskSql).all(...tParams);

  // 各任务完成小计
  const byTask = await Promise.all(tasks.map(async (t) => {
    const recs = await db.prepare('SELECT status FROM completion_record WHERE task_id=?').all(t.id);
    const done = recs.filter((r) => r.status === 'DONE_ONTIME' || r.status === 'DONE_OVERDUE').length;
  ...
  const unfRows = await db.prepare(unfSql).all(...uParams);
  const map = new Map();
  unfRows.forEach((r) => {
  ...
  const totalStudents = await (await db.prepare('SELECT COUNT(*) AS c FROM student').get()).c;
...
router.get('/credit-trend', async (req, res) => {
  ...
  const list = await db.prepare('SELECT * FROM credit_flow WHERE student_id=? ORDER BY create_time').all(studentId);
  const base = await db.prepare('SELECT total_credits FROM student WHERE id=?').get(studentId);
...
router.get('/drill/subject', async (req, res) => {
  ...
  const tasks = await db.prepare('SELECT * FROM task WHERE subject_id=?').all(subjectId);
  const rows = await Promise.all(tasks.map(async t => {
    let recs = await db.prepare('SELECT cr.*, s.name AS studentName FROM completion_record cr LEFT JOIN student s ON cr.student_id=s.id WHERE cr.task_id=?').all(t.id);
...
router.get('/drill/status', async (req, res) => {
  ...
  const rows = await (await db.prepare(sql).all(...params)).map(r => ({ taskTitle: r.taskTitle, studentName: r.studentName, status: STATUS_LABEL[r.status] || r.status }));
  ok(res, rows);
});
