...
router.post('/wx-login', async (req, res) => {
  try {
    ...
    // 1) 已有 openid → 直接登录
    const existing = await db.prepare('SELECT * FROM sys_user WHERE openid=?').get(openid);
    if (existing) {
      // 可选：更新昵称头像
      if ((nickname && !existing.name) || (avatarUrl && !existing.avatar)) {
        await db.prepare('UPDATE sys_user SET name=COALESCE(?,name), avatar=COALESCE(?,avatar) WHERE id=?')
          .run(nickname || null, avatarUrl || null, existing.id);
      }
      const u = await db.prepare('SELECT * FROM sys_user WHERE id=?').get(existing.id);
    ...
    if (dup) {
      ...
      await db.prepare('UPDATE sys_user SET openid=? WHERE id=?').run(openid, dup.id);
    } else {
      const name = nickname || '微信用户';
      const info = await db.prepare(`INSERT INTO sys_user(username, password, name, role, class_id, student_id, openid, avatar)
                  VALUES(?,?,?,?,?,?,?,?)`)
    ...
    const u = await db.prepare('SELECT * FROM sys_user WHERE openid=?').get(openid);
    const token = signToken(userPayload(u));
    return res.json({
