...
router.post('/', authMiddleware, requireRole('ADMIN', 'TEACHER'), async (req, res) => {
  ...
  if (!name) return fail(res, 400, '请填写兴趣分类名称');
  const r = await db.prepare('INSERT INTO subject(name, class_id, teacher_id) VALUES(?,?,?)').run(name, classId || req.user.class_id || 1, teacherId || null);
...
router.put('/:id', authMiddleware, requireRole('ADMIN', 'TEACHER'), async (req, res) => {
  ...
  if (!before) return fail(res, 404, '兴趣分类不存在');
